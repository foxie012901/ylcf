...
const mHeight = Dimensions.get('window').height;
 class ShangJia extends Component {
  ...
  /*  if(DateUtil.time_range(JSON.parse(response).data.businessHours.split("-")[0],JSON.parse(response).data.businessHours.split("-")[1],DateUtil.formatDate(new Date().getTime(),'hh:mm:ss'))){
    ...
    let ShangJiaPage=( <View style={styles.content}>
    ...
    <View style={styles.storeMessageView}>
      ...
      <View style={{backgroundColor:'#FFFFFF',width:1,height:mHeight*0.2,flexDirection:'row'}}>
        <View style={{width:1,backgroundColor:'#B0B0B0',height:mHeight*0.15,alignSelf:'center'}}></View>
      </View>
    <View style={{flex:1,borderTopEndRadius:10,borderBottomEndRadius:10,backgroundColor:'#FFFFFF'}}>
...
const styles = StyleSheet.create({
  ...
  untAndtImage:{
    height:mHeight*0.06,width:mWidth*0.14,borderTopRightRadius:10,alignSelf:'flex-end',resizeMode:'stretch'
  }
