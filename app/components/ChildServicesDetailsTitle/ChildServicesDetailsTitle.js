...
import IconFont from 'react-native-vector-icons/Ionicons'
 class ChlidServicesDetailsTitle extends Component {
    ...
    _carListRenderItem({item,index}){
        ...
        index===0?sty={width:mWidth*0.25,height:mHeight*0.06,position:'relative'}:sty={width:mWidth*0.25,height:mHeight*0.06,position:'relative',borderTopColor:'#cccccc',borderTopWidth:1}

        return(<TouchableOpacity  onPress={()=>{this.props._changeTitle(item.hphm) ,this.props._changeSelectShow(false)}}><View style={sty}><Text style={{textAlign:'center',lineHeight:mHeight*0.06}}>{item.hphm}</Text></View></TouchableOpacity>)
    }
    render() {
        ...
        return (
            ...
            <View style={{width:mWidth,height:mHeight*0.1,backgroundColor:"#FFFFFF",flexDirection:'row',borderBottomWidth:2,justifyContent:'flex-start',}}>       
                ...
                <TouchableHighlight onPress={()=>{this.props.carList.length>0?this.props._changeSelectShow(true):null}} style={{alignSelf:'center',flex:1}}>
